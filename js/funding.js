      ...
      <div class="funding-card-content">
        ...
        <div class="funding-details">
          <div class="funding-detail">
            <span class="detail-icon">≡ƒÆ░</span>
            <div>
          ...
          <div class="funding-detail">
            <span class="detail-icon">Γù╖</span>
            <div>
  ...
  if (!docs.length) {
    $("fundingGrid").innerHTML = `
      <div class="no-results">
        <div class="no-results-icon">Γîò</div>
        <h3>${activeSection === "saved" ? "No saved opportunities" : "No funding opportunities found"}</h3>
        <p>${activeSection === "saved" ? "Click the ΓÖí icon on a funding card to save an opportunity." : "Try changing your search keywords or filters."}</p>
      </div>
    `;
    return;
  }
...
function setSection(section) {
  ...
  $("dashboardMain").innerHTML = `
    ...
    <div class="no-results" style="margin-top:30px">
      <div class="no-results-icon">Γ£ª</div>
      <h3>${title}</h3>
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  $("sortButton").addEventListener("click", () => {
    ...
    $("sortButton").textContent =
      sortAscending ? "Sort by Match Γåæ" : "Sort by Match Γåô";
    renderCards();
  });
